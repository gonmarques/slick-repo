...
import com.byteslounge.slickrepo.mysql.repository.{CarRepository, CoffeeRepository, PersonRepository}
import slick.driver.MySQLDriver.api._
import slick.lifted.TableQuery

class MySQLRepositoryTest extends MySQLAbstractRepositoryTest {
  ...
  it should "rollback a transaction" in {
    ...
    try {
      ...
      case sqle: SQLException if sqle.getErrorCode == 1062 =>
  ...
  it should "pessimistic lock entities" in {
    ...
    val person1: Person = executeAction(personRepository.save(Person(None, "john1")))
    val person2: Person = executeAction(personRepository.save(Person(None, "john2")))

    def runnable(threadNum: Int) = new Runnable() {
      def run() = {
        startLatch.await()
        val work = for {
          x <- personRepository.lock(if(threadNum == 1) person1 else person2)
          y <- DBIO.successful(Thread.sleep(1000))
          z <- personRepository.lock(if(threadNum == 1) person2 else person1)
        } yield (x, y, z)
        try{
          executeAction(personRepository.executeTransactionally(work))
          successCount.incrementAndGet()
        } catch {
          case sqle: SQLException if sqle.getErrorCode == 1213 => failureCount.incrementAndGet()
          case e: Exception                                    => fail
        }
